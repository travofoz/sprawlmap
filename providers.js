export const PROVIDERS = {
  ...
  openrouter: {
    ...
    default:'meta-llama/llama-3-8b-instruct:free',format:'openai',keyHint:'sk-or-... (optional for free models)',
...
export class LLM {
  constructor({provider='openrouter',apiKey='',model,accountId,proxyUrl}={}){
    ...
    this.provider=provider;this.apiKey=apiKey;
  ...
  async chat(messages,system='',maxTokens=1000){
    ...
    if(format==='anthropic'){
      ...
      body={messages:system?[{role:'system',content:system},...messages]:messages,max_tokens:maxTokens};
    }else{
      headers={'Content-Type':'application/json','Authorization':`Bearer ${this.apiKey}`,...(this.cfg.extraHeaders||{})};
      body={model:this.model,messages:system?[{role:'system',content:system},...messages]:messages,max_tokens:maxTokens};
    }
    const r=await fetch(url,{method:'POST',headers,body:JSON.stringify(body)});
    if(!r.ok){const e=await r.text();throw new Error(`${this.provider} ${r.status}: ${e.slice(0,200)}`);}
    return this._extract(await r.json(),format);
  }
  _extract(d,format){
    if(format==='anthropic')return d.content?.find(b=>b.type==='text')?.text||'';
    if(format==='cloudflare')return d.result?.response||d.response||'';
